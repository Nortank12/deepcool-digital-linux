...
impl Gpu {
    ...
    /// Reads the value of the GPU fan speed in RPM.
    pub fn get_fanspeed(&self) -> u16 {
        let data = read_to_string(format!("{}/fan1_input", &self.hwmon_dir)).unwrap_or_else(|_| {
            error!("Failed to get GPU fan speed (AMD)");
            exit(1);
        });

        data.trim_end().parse::<u16>().unwrap()
    }
}
